...
/*
  ...
  Поле header отвечает на вопрос «влезает ли в одну строку наверху».

  Замер 26 августа при ширине окна 1024 (с неё начинается настольное меню):
  строка шапки 1009 точек, занято 898, свободно 111. Один пункт стоит около 70,
  то есть восьмой влезает с запасом, а девятый уже нет.

  Прежняя редакция этого комментария утверждала обратное — «под пункты остаётся
  около 470 точек, семь занимают 453, восьмой не влезает». Число 470 я взял из
  головы, не измерив: считал ширину пунктов, а не всей строки, и не вычел логотип
  с кнопкой справа. Из-за этой ошибки академия сначала попала только в подвал.
  Замер делается по ЖИВОЙ странице, а не по прикидке.
*/
...
export const SECTIONS: Раздел[] = [
  ...
  { label: 'Academy', page: 'academy', href: '/academy', header: true },
