...
/**
 * Контраст по WCAG 2.1. Порог 4.5 для обычного текста, 3.0 для крупного и
 * для заливок элементов управления.
 *
 * Проверяются ТОЛЬКО те пары, которые реально встречаются на сайте. Замер
 * 24 августа показал, зачем эта оговорка: расчёт по всем сочетаниям токенов
 * дал пять «провалов», из которых три оказались теоретическими. Токен
 * accent-dark не использовался нигде вообще (объявлен и забыт, теперь убран),
 * а accent-from и accent-to живут только внутри градиента, где текста нет.
 *
 * Сторож, ругающийся на несуществующие сочетания, заставляет чинить то, чего
 * никто не видит, и заодно приучает не читать его вывод.
 */
export function контраст(hex1, hex2) {
  const яркость = (hex) => {
  ...
  const [светлее, темнее] = [яркость(hex1), яркость(hex2)].sort((a, b) => b - a);
  return (светлее + 0.05) / (темнее + 0.05);
}

/** Пары, встречающиеся на сайте. Проверено замером в браузере 24.08.2026. */
export const ПАРЫ_ЦВЕТОВ = [
  ['светлая: обычный текст на фоне', '#000000', '#EEF1F0', 4.5],
  ['светлая: второй текст на фоне', '#555555', '#EEF1F0', 4.5],
  ['светлая: второй текст на белой карточке', '#555555', '#ffffff', 4.5],
  ['светлая: акцентный текст на фоне', '#06794F', '#EEF1F0', 4.5],
  ['светлая: акцентный текст на белой карточке', '#06794F', '#ffffff', 4.5],
  ['светлая: акцент как заливка кнопки', '#089662', '#EEF1F0', 3.0],
  ['тёмная: белый текст на фоне', '#ffffff', '#191919', 4.5],
  ['тёмная: второй текст на фоне', '#cccccc', '#191919', 4.5],
  ['тёмная: акцентный текст на фоне', '#0AA76D', '#191919', 4.5],
  ['тёмная: акцентный текст на карточке', '#0AA76D', '#202020', 4.5],
  ['тёмная: акцентный текст на поверхности', '#0AA76D', '#2a2a2a', 4.5],
];
...
async function selfTest() {
  ...
  // ── Контраст встречающихся на сайте пар ──────────────────────────────
  for (const [имя, текст, фон, порог] of ПАРЫ_ЦВЕТОВ) {
    const r = контраст(текст, фон);
    t(`контраст ${имя} = ${r.toFixed(2)} (нужно ${порог})`, () => r >= порог);
  }
  const палитра = await readFile(path.join(ROOT, 'index.css'), 'utf8');
  t('акцентный текст тёмной темы в палитре совпадает с проверенным', () =>
    палитра.includes('--color-accent-text: #0AA76D'));
