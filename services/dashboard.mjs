...
function страница(сырые, когда) {
  ...
  */
  ...
  /*
    Устройство меню взято из Project X (Mosco-corp/web): плоские пункты
    сверху, раскрывающиеся группы, отдельная нижняя секция и сворачивание в
    узкую полосу. Наполнение — наше, и его мы ещё будем продумывать.
  */
  const меню =
    ...
    группа({
      ...
      имя: 'Справочники',
      открыта: false,
      дети: пункт({ адрес: 'catalog', имя: 'Каталог услуг', метка: String(Object.keys(УСЛУГИ).length), значок: ЗНАЧКИ.каталог }),
    });
