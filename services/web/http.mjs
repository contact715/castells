...
import { принятьПосещение, доменыПроектов, проектПоКарточкеYelp } from '../app/counter.mjs';
import { записатьСнимокYelp } from '../store/yelp.mjs';
import { СКРИПТ_СЧЁТЧИКА } from './counter-script.mjs';
import { страницаСнимков } from './snapshot-page.mjs';
...
/*
  УЗКИЙ КЛЮЧ ТОЛЬКО ДЛЯ СНИМКОВ.

  Снимок карточки Yelp снимает закладка в браузере — человек открывает карточку
  сам и нажимает кнопку. Ключ при этом лежит в тексте закладки, то есть в
  браузере, и однажды его кто-нибудь скопирует вместе с закладкой или увидит
  через плечо.

  Поэтому это ОТДЕЛЬНЫЙ ключ, а не пароль пульта: он даёт право ровно на одно
  действие — прислать снимок, — и не открывает ни страницу, ни данные клиентов.
  Утечка такой закладки стоит «кто-то может прислать нам мусорный снимок», а не
  «кто-то читает наши проекты».

  Почему автоматического сбора нет вовсе: Yelp отвечает 403 и headless-браузеру,
  и обычному браузеру под управлением Playwright — проверено 30 августа. Обходить
  их защиту от ботов мы не станем, поэтому страницу открывает человек.
*/
const КЛЮЧ_СНИМКОВ = process.env.YELP_SNAPSHOT_TOKEN || '';
...
export async function обработать(запрос, ответ) {
  ...
  */
  ...
  /*
    ПРИЁМ СНИМКА КАРТОЧКИ YELP.

    Стоит ДО проверки пароля пульта и имеет СВОЙ узкий ключ. Причина: снимок
    присылает закладка, которую человек нажимает прямо на странице Yelp, — она
    приходит с чужого домена и пароля пульта знать не должна.

    Автоматически собирать эти данные нельзя: Yelp отвечает 403 и headless-
    браузеру, и обычному под управлением Playwright (проверено 30 августа).
    Обходить защиту от ботов мы не станем, поэтому карточку открывает человек,
    а закладка только снимает уже открытую страницу.
  */
  if (путь === '/api/yelp-snapshot') {
    const источникЗапроса = запрос.headers.origin || '';
    /* Разрешаем ровно Yelp: закладка работает там и больше нигде. */
    const свой = /(^|\.)yelp\.com$/i.test(домен(источникЗапроса));
    const шапкаCORS = свой
      ? { 'Access-Control-Allow-Origin': источникЗапроса, 'Vary': 'Origin' }
      : {};
    ...
    /* Пускают два ключа: узкий (закладка) и общий пароль пульта (инструмент
       командной строки). Оба сравниваются за постоянное время. */
    const заголовок = (запрос.headers.authorization || '').replace(/^Bearer /, '');
    const можно = (КЛЮЧ_СНИМКОВ && равныБезопасно(заголовок, КЛЮЧ_СНИМКОВ))
      || (ПАРОЛЬ && равныБезопасно(заголовок, ПАРОЛЬ));
    if (!можно) {
      return отдать(ответ, 401, 'application/json; charset=utf-8',
        JSON.stringify({ принято: false, причина: 'нужен ключ снимков' }), шапкаCORS);
    }
    ...
    try { снимок = JSON.parse(тело); } catch {
      return отдать(ответ, 400, 'application/json; charset=utf-8',
        JSON.stringify({ принято: false, причина: 'тело не разобралось как JSON' }), шапкаCORS);
    }

    /* Адрес карточки обязателен: без него неизвестно, чей это снимок. Сам
       проект можно не указывать — сервер найдёт его по адресу среди профилей,
       чтобы закладка сбора работала одна на всех клиентов. */
    if (!снимок?.адресYelp) {
      return отдать(ответ, 400, 'application/json; charset=utf-8',
        JSON.stringify({ принято: false, причина: 'нужно поле «адресYelp»' }), шапкаCORS);
    }
    if (!снимок.проект) {
      снимок.проект = await проектПоКарточкеYelp(снимок.адресYelp);
    }
    if (!снимок.проект) {
      return отдать(ответ, 404, 'application/json; charset=utf-8',
        JSON.stringify({
          принято: false,
          причина: 'эта карточка Yelp не привязана ни к одному клиенту — впишите её адрес в поле «yelp» профиля',
        }), шапкаCORS);
    }

    const записано = await записатьСнимокYelp(снимок);
    return отдать(ответ, записано ? 201 : 503, 'application/json; charset=utf-8',
      JSON.stringify({ принято: записано, проект: снимок.проект }), шапкаCORS);
  }
  ...
  try {
    /*
      Страница-помощник для снятия снимков Yelp. ЗА ПАРОЛЕМ пульта: внутри
      лежит ключ снимков, и отдавать его кому попало незачем.
    */
    if (путь === '/snapshot') {
      const проекты = await собратьПроекты();
      const хост = запрос.headers.host || '';
      const схема = (запрос.headers['x-forwarded-proto'] || 'https').split(',')[0];
      return отдать(ответ, 200, 'text/html; charset=utf-8', страницаСнимков({
        адресПульта: `${схема}://${хост}`,
        /* Если узкий ключ не задан, закладка работает по паролю пульта: лучше
           рабочая закладка с общим ключом, чем нерабочая с пустым. Разница
           названа на самой странице сервером через переменные, а не молчанием. */
        ключ: process.env.YELP_SNAPSHOT_TOKEN || ПАРОЛЬ,
        проекты,
      }));
    }
