...
export function ChatDemo({ initialMessage }: { initialMessage: string }) {
  ...
  return (
    <div className="flex flex-col h-[500px] rounded-[2rem] overflow-hidden bg-bblack/20 backdrop-blur-sm">
      {/* Header */}
      <div className="p-4  bg-black/5 dark:bg-dark-surface/50 flex items-center gap-2">
        <div className="relative">
          <div className="w-2 h-2 rounded-full bg-green-500 absolute bottom-0 right-0 black/50" />
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-4" ref={scrollRef}>
        <AnimatePresence initial={false}>
          {messages.map((msg) => (
            ...
            >
              <div
                className={`max-w-[80%] rounded-[2rem] px-4 py-2.5 text-sm ${msg.role === "user"
                  ? "bg-coral text-white rounded-br-none"
                  : "bg-black/5 dark:bg-white/10 text-white/90 rounded-bl-none"
                  }`}
              >
                {msg.text}
              </div>
        ...
        {isTyping && (
          ...
          >
            <div className="bg-black/5 dark:bg-dark-surface/50 rounded-[2rem] rounded-bl-none px-4 py-3 flex gap-1">
      ...
      <div className="p-4  bg-black/5 dark:bg-dark-surface/50">
        ...
        >
          <input
            ...
            className="flex-1 bg-bblack/20 rounded-full px-4 py-2 text-sm focus:outline-none focus:ring-coral/50 transition-colors"
          />
          <Button
            type="submit"
