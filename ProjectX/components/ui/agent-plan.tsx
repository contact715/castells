...
export default function Plan() {
    ...
    return (
        <div className="bg-transparent text-foreground h-full overflow-auto p-2">
            <motion.div
                className="bg-[#11141D] border-white/5 rounded-[2rem] border shadow-2xl overflow-hidden"
                        ...
                        <ul className="space-y-4 overflow-hidden">
                            {tasks.map((task, index) => {
                                ...
                                return (
                                    <motion.li
                                        key={task.id}
                                        className={` ${index !== 0 ? "mt-4 pt-4 border-t border-white/5" : ""} `}
                                        initial="hidden"
                                        animate="visible"
                                    ...
                                    >
                                        {/* Task row */}
                                        <motion.div
                                            className="group flex items-center px-4 py-3 rounded-2xl hover:bg-white/5 transition-all duration-300"
                                        >
                                            <motion.div
                                                ...
                                                <AnimatePresence mode="wait">
                                                    ...
                                                    >
                                                        {task.status === "completed" ? (
                                                            ...
                                                            <CircleDotDashed className="h-6 w-6 text-blue-500 animate-spin-slow" />
                                                        ) : task.status === "need-help" ? (
                                                            <CircleAlert className="h-6 w-6 text-yellow-500" />
                                                        ) : task.status === "failed" ? (
                                                            <CircleX className="h-6 w-6 text-red-500" />
                                                        ) : (
                                                            <Circle className="text-white/20 h-6 w-6" />
                                                        )}
                                                    </motion.div>
                                                </AnimatePresence>
                                            </motion.div>
                                            ...
                                            >
                                                <div className="mr-4 flex-1 truncate">
                                                    <span
                                                        className={`text-lg font-bold tracking-tight ${isCompleted ? "text-white/40 line-through" : "text-white"}`}
                                                    >
                                                        {task.title}
                                                    </span>
                                                </div>
                                                ...
                                                <div className="flex flex-shrink-0 items-center space-x-3 text-xs">
                                                        ...
                                                        <div className="flex items-center mr-4">
                                                            <div className="flex flex-wrap gap-1.5">
                                                                {task.dependencies.map((dep, idx) => (
                                                                    <motion.span
                                                                        key={idx}
                                                                        className="bg-white/5 text-white/40 rounded px-2 py-0.5 text-[9px] font-black uppercase tracking-widest border border-white/5"
                                                    ...
                                                    <motion.span
                                                        className={`rounded-lg px-3 py-1 text-[10px] font-black uppercase tracking-widest ${task.status === "completed"
                                        ...
                                        <AnimatePresence mode="wait">
                                            {isExpanded && task.subtasks.length > 0 && (
                                                ...
                                                >
                                                    ...
                                                    <div className="absolute top-0 bottom-0 left-[27px] border-l border-dashed border-white/10" />
                                                    <ul className="mt-2 mr-2 mb-2 ml-4 space-y-1">
                                                        {task.subtasks.map((subtask) => {
                                                            ...
                                                            return (
                                                                ...
                                                                >
                                                                    <motion.div
                                                                        className="flex flex-1 items-center rounded-xl p-2 hover:bg-white/[0.03] transition-colors"
                                                                        layout
                                                                    >
                                                                        <motion.div
                                                                            ...
                                                                            <AnimatePresence mode="wait">
                                                                                ...
                                                                                >
                                                                                    {subtask.status === "completed" ? (
                                                                                        ...
                                                                                        <Circle className="text-white/10 h-4 w-4" />
                                                                                    )}
                                                                                </motion.div>
                                                                            </AnimatePresence>
                                                                        </motion.div>
                                                                        ...
                                                                        <span
                                                                            className={`text-sm font-medium tracking-wide ${subtask.status === "completed" ? "text-white/30 line-through" : "text-white/70"}`}
                                                                        >
                                                                            {subtask.title}
                                                                        </span>
                                                                    </motion.div>
                                                                    ...
                                                                    <AnimatePresence mode="wait">
                                                                        {isSubtaskExpanded && (
                                                                            <motion.div
                                                                                className="text-white/40 border-white/10 mt-1 ml-2 border-l border-dashed pl-6 text-[13px] overflow-hidden"
                                                                            ...
                                                                            >
                                                                                ...
                                                                                {subtask.tools && subtask.tools.length > 0 && (
                                                                                    <div className="mt-2 mb-3 flex flex-wrap items-center gap-2">
                                                                                        <span className="text-[10px] uppercase font-black tracking-widest text-[#E8D5B5]/60 pr-1">
                                                                                            ACTIVE MCP ENGINES:
                                                                                        </span>
                                                                                        <div className="flex flex-wrap gap-1.5">
                                                                                            {subtask.tools.map((tool, idx) => (
                                                                                                <motion.span
                                                                                                    key={idx}
                                                                                                    className="bg-white/5 text-white/50 rounded-md px-2 py-0.5 text-[10px] font-bold border border-white/5 shadow-inner"
