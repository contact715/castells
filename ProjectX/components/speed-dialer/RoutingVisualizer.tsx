...
export function RoutingVisualizer({ active = false, onComplete }: { active: boolean; onComplete?: () => void }) {
    ...
    return (
        <div className="relative w-full h-48 bg-black/5 dark:bg-white/5 rounded-xl border border-black/5 dark:border-white/5 overflow-hidden flex items-center justify-center p-8">
            {/* Connecting Lines */}
            ...
            <div className="relative w-full flex justify-between items-center z-10">
                ...
                <div className="flex flex-col items-center gap-2">
                    <motion.div
                        className={`w-12 h-12 rounded-full flex items-center justify-center border-2 transition-colors duration-300 ${step >= 1 ? "bg-coral border-coral text-white" : "bg-white/5 border-white/10 text-white/40"
                            }`}
                ...
                <div className="flex flex-col items-center gap-2">
                    <motion.div
                        className={`w-16 h-16 rounded-xl flex items-center justify-center border-2 transition-colors duration-300 ${step >= 2 ? "bg-blue-600 border-blue-500 text-white" : "bg-white/5 border-white/10 text-white/40"
                            }`}
                        animate={step === 2 ? { scale: [1, 1.1, 1], rotate: [0, 5, -5, 0] } : {}}
                    >
                        <Server className="w-8 h-8" />
                    </motion.div>
                    <span className={`text-xs font-bold uppercase tracking-wider ${step >= 2 ? "text-blue-500" : "text-white/40"}`}>AI Filter</span>
                </div>

                {/* Node 3: Manager */}
                <div className="flex flex-col items-center gap-2">
                    <motion.div
                        className={`w-12 h-12 rounded-full flex items-center justify-center border-2 transition-colors duration-300 ${step >= 3 ? "bg-green-600 border-green-500 text-white" : "bg-white/5 border-white/10 text-white/40"
                            }`}
